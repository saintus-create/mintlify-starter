...
You cannot use `api` for the name of a folder unless you nest it inside another folder. Documentation Platform uses Next.js which reserves the top-level `api` folder for internal server calls. A folder name such as `api-reference` would be accepted.
